...
const PrivateProfileUpdate = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    dispatch(startLoading({ message: 'Updating profile...', animationType: 'wave' }));
    ...
    try {
      await dispatch(updatePrivateProfile(data)).unwrap();
      await dispatch(fetchUserProfile()).unwrap();
      dispatch(showSnackbar({
        message: (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography>Profile updated successfully!</Typography>
            <Button size="small" onClick={() => navigate(`/profile/${profile._id || profile.user?._id}`)} sx={{ ml: 1 }}>
              View Public Profile
            </Button>
          </Box>
        ),
