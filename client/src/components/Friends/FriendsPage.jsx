...
const FriendsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all' or 'mutual'
  ...
  const isOwnProfile = profileUserId === String(currentUserId);

  useEffect(() => {
    if (activeTab === 'all') {
      dispatch(fetchUserFriends({ userId: profileUserId, page }));
    } else {
      dispatch(fetchMutualFriends({ userId: profileUserId, page }));
    }
  }, [dispatch, profileUserId, page, activeTab]);
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <Chip
          label={`${
            activeTab === 'all'
              ? friendsList.pagination.totalItems
              : mutualFriends.pagination.totalItems
          } total`}
          sx={{ ml: 2 }}
          color="primary"
        />
      ...
      <Box sx={{ mb: 3 }}>
        ...
        {!isOwnProfile && (
          <Button
            variant={activeTab === 'mutual' ? 'contained' : 'outlined'}
            onClick={() => handleTabChange(null, 'mutual')}
          >
            Mutual Friends
          </Button>
        )}
...
const FriendItem = ({ friend, isOwnProfile }) => (
  <Grid item xs={12} sm={6} md={4}>
    ...
    >
      <Avatar src={friend.profileImage} sx={{ width: 56, height: 56, mr: 2 }} />
      <Box sx={{ flexGrow: 1 }}>
