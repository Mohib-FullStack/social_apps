...
const handleProcessRegister = async (req, res, next) => {
  try {
    ...
    const { exists: phoneExists } = phone ? await checkUserExist({ phone }) : { exists: false };
...
const handleActivateUser = async (req, res, next) => {
  try {
    ...
    if (decoded.profileImagePath) {
      try {
        const response = await cloudinary.uploader.upload(decoded.profileImagePath, {
...
const handleFetchUserProfile = async (req, res, next) => {
  try {
    ...
    console.log('Attempting to fetch user profile from DB for user ID:', userId)
...
const handleUpdateUserById = async (req, res, next) => {
  try {
    ...
    const allowedFields = [
      ...
      'profileImage', 'coverImage', 'bio', 'website', 
      'gender', 'birthDate', 'privacySettings'
    ];
    ...
    if (req.file) {
      try {
        ...
        if (user.profileImage) {
          const deletionSuccess = await deleteFileFromCloudinary(user.profileImage);
          if (!deletionSuccess) {
            console.warn('Old image could not be deleted, but continuing with update');
          }
        }
    ...
    if (filteredUpdateData.gender && filteredUpdateData.gender !== user.gender) {
    ...
    const sensitiveFields = [
      ...
      'passwordResetExpires', 'emailVerifyToken'
    ];
...
if (process.env.NODE_ENV === 'development') {
 ...
 // this is testing end
    ...
    if (user.isBanned) {
      await transaction.rollback();
      return errorResponse(res, 'Banned users cannot change phone numbers', 403);
    }

    // Check if phone is unchanged
    if (normalizedPhone === user.phone) {
      await transaction.rollback();
      return errorResponse(res, 'This is already your current phone number', 400);
    }
    ...
    if (existingRequest) {
      ...
      if (new Date() < existingRequest.expiresAt) {
        ...
        return errorResponse(res, {
          ...
          canResendAt: new Date(existingRequest.createdAt.getTime() + OTP_CONFIG.resendDelay * 60 * 1000),
          remainingTime: Math.ceil((existingRequest.expiresAt - new Date()) / 1000 / 60)
        }, 429);
      }
    ...
    if (!smsResult.success) {
      ...
      if (smsResult.error?.includes('invalid')) {
        return errorResponse(res, 'Invalid phone number: Cannot receive SMS', 400);
      }
...
const verifyPhoneOTP = async (req, res) => {
  let transaction;
  try {
    console.log('[OTP Verification] Starting verification process for user:', req.user.id);
    ...
    console.error('[OTP Verification] CRITICAL ERROR:', {
      ...
      userId: req.user?.id,
      body: req.body,
      dbConnectionStatus: sequelize.authenticate ? 'checking...' : 'no connection'
    });
  ...
  try {
    ...
    if (user.profileImage) {
      try {
        ...
        deletionResults.push({ type: 'profileImage', success: false, error: error.message });
      }
    }
    ...
    if (user.coverImage) {
      try {
        ...
        deletionResults.push({ type: 'coverImage', success: false, error: error.message });
      }
    }
