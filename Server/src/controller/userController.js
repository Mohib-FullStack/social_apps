...
const handleGetPrivateProfile = async (req, res, next) => {
  try {
    ...
    // Successful response with isOwnProfile: true
    return successResponse(res, {
      statusCode: 200,
      message: 'User profile retrieved successfully',
      payload: { 
...
const handleGetPublicProfile = async (req, res, next) => {
  try {
    const rawId = req.params.id;
    const requestingUserId = req.user?.id; // Get the current user's ID if authenticated
    ...
    // 6️⃣ Build public payload with isOwnProfile flag
    const publicData = {
      id: user.id,
      firstName: user.firstName,
      lastName: user.lastName,
      ...
      createdAt: user.createdAt,
      updatedAt: user.updatedAt,
      isOwnProfile: requestingUserId ? requestingUserId === userId : false,
    };
...
// const { handledestroyAllSessions } = require('../controller/authController');
// const TempPhoneUpdate = require('../models/tempPhoneUpdateModel');
// const verificationDocumentModel = require('../models/verificationDocumentModel');
// const User = require('../models/userModel');
...
// const getAllUsersWithVerificationDocs = async (req, res) => {
//   try {
//     const users = await User.findAll({
//       include: [
//         {
//           model: verificationDocumentModel,
//           as: 'userVerificationDocuments',
//         }
//       ]
//     });
...
//   } catch (error) {
//     console.error("Failed to fetch users with verification documents:", error);
...
//   handleBanUserById,
//   handleUnbanUserById,
//   getAllUsersWithVerificationDocs,
