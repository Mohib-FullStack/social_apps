...
const initializeSocketIO = async (httpServer, redisConfig) => {
  ...
  io.on('connection', (socket) => {
    logger.info(
      `User connected: ${socket.user ? socket.user.id : 'guest'} (Socket: ${
        socket.id
      })`
    );
    ...
    socket.on(
      'validate-field',
      async ({ field, value, requestId }, callback) => {
    ...
    if (socket.user) {
      ...
      socket.on('sendMessage', async (data, callback) => {
        try {
          ...
          logger.info(
            `Message sent in chat ${chatId} by user ${socket.user.id}`
          );
        } catch (error) {
          logger.error(
            `Error sending message by user ${socket.user.id}:`,
            error.message
          );
      ...
      socket.on(
        'markMessagesRead',
        async ({ chatId, messageIds }, callback) => {
          try {
            ...
            logger.info(
              `User ${socket.user.id} marked messages as read in chat ${chatId}`
            );
          } catch (error) {
            logger.error(
              `Error marking messages read by user ${socket.user.id}:`,
              error.message
            );
    ...
    socket.on('error', (error) => {
      logger.error(
        `Socket error for user ${socket.user ? socket.user.id : 'guest'}:`,
        error.message
      );
    });
  });
