...
PendingGenderChange.init({
  ...
  requestedGender: {
    ...
    allowNull: false,
    validate: { notNull: { msg: 'Requested gender is required' } }
  },
  currentGender: {
    type: DataTypes.ENUM('male', 'female', 'other'),
    allowNull: false,
    validate: { notNull: { msg: 'Current gender is required' } }
  },
  verificationToken: {
    type: DataTypes.TEXT,
    allowNull: false,
  ...
  reason: {
    ...
    allowNull: true,
    validate: {
      len: { args: [0, 500], msg: 'Reason must be less than 500 characters' },
  ...
  reviewNotes: {
    ...
    allowNull: true,
    validate: {
      len: { args: [0, 1000], msg: 'Review notes must be less than 1000 characters' }
    }
  },
  reviewedBy: {
    type: DataTypes.INTEGER,
  ...
  adminAlertId: {
    type: DataTypes.INTEGER,
    allowNull: true,
    references: { model: 'admin_alerts', key: 'id' },
    onDelete: 'SET NULL'
  }
}, {
  sequelize,
  modelName: 'PendingGenderChange',
